...
export const Route = createFileRoute("/_authenticated/grading")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Valutazioni di grading con probabilità PSA 6-10, expected value e profitto atteso.",
      },
    ...
    <AppShell title="Grading" subtitle={`${rows.length} valutazioni`}>
      {rows.length === 0 ? (
        <p className="text-sm text-muted-foreground">
          Nessuna valutazione di grading registrata.
        </p>
      ) : (
                ...
                return (
                  <TableRow key={i.id}>
                    ...
                    <TableCell>
                      {g?.recommendation ? (
                        <Badge
                          variant={g.recommendation === "GRADA" ? "default" : "secondary"}
                        >
                          {g.recommendation}
                        </Badge>
                      ) : (
